        ...
        for item in valid_freq_files:
            with io.open(item, 'r') as rif:
                for line in rif:
                    ...
                    # get 4gram frequency TOTAL
                    # numerator --> number 4gram seen in THIS file, for THIS author
                    # denominator --> number 4gram seen in ALL files of THIS author
...
timing0_end = time.time()
timing0_elapsed = round((timing0_end - timing0_start), 8)
print("DONE -2a- Save-ind-consistency: '%s' - '%s'" % (timing0_elapsed, human_elapsed(timing0_elapsed),))
